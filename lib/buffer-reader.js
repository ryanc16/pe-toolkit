...
function BufferReader(buffer) {
    ...
    this.pushOffset = function() {
        this.offsetStack.push(this.offset);
    }

    this.popOffset = function() {
    ...
    this.slicedBuffer = function(offset=this.offset, byte_count) {
        return new BufferReader(this.data.buffer.slice(offset, offset + byte_count));
    }

    this.readBytes = function(byte_count) {
        const bytes = new Uint8Array(this.data.buffer.slice(this.offset, this.offset + byte_count));
        this.offset += (bytes.length * DATA_TYPES.BYTE);
        return bytes;
    }
    ...
    this.writeWords = function(words) {
    ...
    this.writeDWord = function(dWord) {
    ...
    this.readBytesAsString = function(byte_count) {
        const bytes = this.readBytes(byte_count);
        const str = [];
    ...
    this.seekNonZeroWord = function() {
        while (this.peekWord(true) === 0x0000) {
            this.readWord();
        }
    }
    ...
    this.findNext = function(data, offset=this.offset) {
        ...
        return SeekLocation(start, end);
    }
    
    /**
     * Find the next location of a word's worth of bytes. If it is not found, it will return null
     * @param {string} data 
     * @param {number} offset 
     * @returns {SeekLocation|null} The located work, or null if it was not found
     */
    this.findNextWord = function(data, offset=this.offset) {
        ...
        return offset >= this.data.byteLength ? null :SeekLocation(offset, offset + word.length/2);
    }
 ...
 * 
 * @typedef {{start: {dec: number, hex: string}, end: {dec: number, hex: string}}} SeekLocation
 * @param {Number} start 
 * @param {Number} end 
 * @returns {SeekLocation} An Object describing the start and end offsets, in both decimal and hex
 */
